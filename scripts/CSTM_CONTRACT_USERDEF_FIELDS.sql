CREATE OR REPLACE VIEW DEVV_STG_FLX.CSTM_CONTRACT_USERDEF_FIELDS_get_delta AS LOCKING ROW FOR ACCESS
SELECT
  ...
  CASE
    ...
    AND (
      ...
      OR (
        (
          X.FIELD_NAME IS NULL AND NOT Y.FIELD_NAME IS NULL
        )
        OR (
          NOT X.FIELD_NAME IS NULL AND Y.FIELD_NAME IS NULL
        )
        OR (
      ...
      OR (
        (
          X.FIELD_VALUE IS NULL AND NOT Y.FIELD_VALUE IS NULL
        )
        OR (
          NOT X.FIELD_VALUE IS NULL AND Y.FIELD_VALUE IS NULL
        )
        OR (
          X.FIELD_VALUE <> Y.FIELD_VALUE
        )
      )
      OR (
        (
  ...
  END AS OPT_FLAG
FROM DEVT_STG_FLX.CSTM_CONTRACT_USERDEF_FIELDS AS X
FULL JOIN DEVT_STG_FLX.CSTM_CONTRACT_USERDEF_FIELDS_BACKUP AS Y
