CREATE OR REPLACE VIEW DEVV_STG_FLX.LDTM_CLUSTER_MASTER_get_delta AS LOCKING ROW FOR ACCESS
SELECT
  ...
  CASE
    ...
    AND (
      ...
      OR (
        (
          X.CLUSTER_DESC IS NULL AND NOT Y.CLUSTER_DESC IS NULL
        )
        OR (
          NOT X.CLUSTER_DESC IS NULL AND Y.CLUSTER_DESC IS NULL
        )
        OR (
          X.CLUSTER_DESC <> Y.CLUSTER_DESC
        )
      )
      OR (
        (
    ...
    THEN 'UPD'
    WHEN X.CLUSTER_ID IS NULL
    THEN 'DEL' /* - IF MULTIPLE PKs are there then concatenate them with AND */
  END AS OPT_FLAG
FROM DEVT_STG_FLX.LDTM_CLUSTER_MASTER AS X
FULL JOIN DEVT_STG_FLX.LDTM_CLUSTER_MASTER_BACKUP AS Y
