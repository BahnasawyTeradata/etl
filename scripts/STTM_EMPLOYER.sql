CREATE OR REPLACE VIEW DEVV_STG_FLX.STTM_EMPLOYER_get_delta AS LOCKING ROW FOR ACCESS
SELECT
  ...
  CASE
    ...
    AND (
      ...
      OR (
        (
          X.BM_EMPLOYER_CODE IS NULL AND NOT Y.BM_EMPLOYER_CODE IS NULL
        )
        OR (
          NOT X.BM_EMPLOYER_CODE IS NULL AND Y.BM_EMPLOYER_CODE IS NULL
        )
        OR (
          X.BM_EMPLOYER_CODE <> Y.BM_EMPLOYER_CODE
        )
      )
      OR (
        (
      ...
      OR (
        (
          X.EMPLOYER_DESC IS NULL AND NOT Y.EMPLOYER_DESC IS NULL
        )
        OR (
          NOT X.EMPLOYER_DESC IS NULL AND Y.EMPLOYER_DESC IS NULL
        )
        OR (
          X.EMPLOYER_DESC <> Y.EMPLOYER_DESC
        )
      )
      OR (
        (
      ...
      OR (
        (
          X.SK_EMPLOYER_CODE IS NULL AND NOT Y.SK_EMPLOYER_CODE IS NULL
        )
        OR (
          NOT X.SK_EMPLOYER_CODE IS NULL AND Y.SK_EMPLOYER_CODE IS NULL
        )
        OR (
          X.SK_EMPLOYER_CODE <> Y.SK_EMPLOYER_CODE
        )
      )
    )
    THEN 'UPD'
    WHEN X.EMPLOYER_CODE IS NULL
    THEN 'DEL' /* - IF MULTIPLE PKs are there then concatenate them with AND */
  END AS OPT_FLAG
FROM DEVT_STG_FLX.STTM_EMPLOYER AS X
FULL JOIN DEVT_STG_FLX.STTM_EMPLOYER_BACKUP AS Y
