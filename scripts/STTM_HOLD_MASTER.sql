CREATE OR REPLACE VIEW DEVV_STG_FLX.STTM_HOLD_MASTER_get_delta AS LOCKING ROW FOR ACCESS
SELECT
  ...
  CASE
    ...
    AND (
      ...
      OR (
        (
          X.HOLD_DESC IS NULL AND NOT Y.HOLD_DESC IS NULL
        )
        OR (
          NOT X.HOLD_DESC IS NULL AND Y.HOLD_DESC IS NULL
        )
        OR (
          X.HOLD_DESC <> Y.HOLD_DESC
        )
      )
      OR (
        (
          X.HOLD_TYPE IS NULL AND NOT Y.HOLD_TYPE IS NULL
        )
        OR (
          NOT X.HOLD_TYPE IS NULL AND Y.HOLD_TYPE IS NULL
        )
        OR (
          X.HOLD_TYPE <> Y.HOLD_TYPE
        )
      )
      OR (
        (
    ...
    THEN 'UPD'
    WHEN X.HOLD_CODE IS NULL
    THEN 'DEL' /* - IF MULTIPLE PKs are there then concatenate them with AND */
  END AS OPT_FLAG
FROM DEVT_STG_FLX.STTM_HOLD_MASTER AS X
FULL JOIN DEVT_STG_FLX.STTM_HOLD_MASTER_BACKUP AS Y
