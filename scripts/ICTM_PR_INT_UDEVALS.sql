CREATE OR REPLACE VIEW DEVV_STG_FLX.ICTM_PR_INT_UDEVALS_get_delta AS LOCKING ROW FOR ACCESS
SELECT
  ...
  CASE
    ...
    AND X.UDE_ID IS NULL
    THEN 'DEL' /* - IF MULTIPLE PKs are there then concatenate them with AND */
  END AS OPT_FLAG
FROM DEVT_STG_FLX.ICTM_PR_INT_UDEVALS AS X
FULL JOIN DEVT_STG_FLX.ICTM_PR_INT_UDEVALS_BACKUP AS Y
